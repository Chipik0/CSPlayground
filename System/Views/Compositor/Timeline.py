...
class ScrollableContent(QGraphicsView):
    ...
    def setup_ui(self) -> None:
        ...
        self.playhead_hover = Widgets.PlayheadItem(self, Styles.Metrics.Tracks.ruler_height + Styles.Metrics.Waveform.height)
    ...
    def scale_view(self, delta: float, force_update: bool = False) -> None:
    ...
    def draw_waveform(self, painter: QPainter, rect: QRectF) -> None:
        ...
        for i in range(start_tile, end_tile + 1):
            ...
            if draw_x <= rect.right() and draw_x + self.tile_width >= rect.left():
                painter.drawPixmap(draw_x, Styles.Metrics.Tracks.ruler_height, tile)
    ...
    def draw_ruler(self, painter: QPainter, rect: QRectF) -> None:
        ...
        for i in range(start_second, end_second + 1):
            ...
            painter.drawText(QPointF(x + 5, Styles.Metrics.Tracks.ruler_height - 10), str(i))
    ...
    def draw_track_grid(self, painter: QPainter, rect: QRectF) -> None:
        ...
        y = (
            Styles.Metrics.Tracks.ruler_height +
            Styles.Metrics.Waveform.height +
            Styles.Metrics.Tracks.box_spacing
        )

        for track_name in self.track_names:
            top_y    = y + (Styles.Metrics.Tracks.row_height - Styles.Metrics.Tracks.box_height) / 2.0
            bottom_y = top_y + Styles.Metrics.Tracks.box_height
    ...
    def contextMenuEvent(self, event: QContextMenuEvent) -> None:
        ...
        try:
            ...
            effect_entries = [
                self.make_effect_entry(name, config, clicked_glyph, selected_ids)
    ...
    def resolve_effect_options(
        ...
        has_non_segmented = [
            not segments_map.get(self.composition.get_glyph(gid)["track"], False)
            for gid in selected_ids
        ]
        
        has_segmented = [
            segments_map.get(self.composition.get_glyph(gid)["track"], False)
            for gid in selected_ids
        ]
        
        has_custom_segments = any(
            GlyphEffects.is_segment_edited(self.composition.get_glyph(gid))
            for gid in selected_ids
        )
        
        same_track = all(
            selected_items[0].track == item.track for item in selected_items
        )
    ...
    def make_effect_entry(
        ...
        preview = Menu.EffectPreviewWidget(effect_name, config, clicked_glyph)
        preview.apply_requested.connect(lambda name = effect_name, settings = config: self.apply_effect_to_selection(name, settings, selected_ids))

        return (effect_name, preview)
