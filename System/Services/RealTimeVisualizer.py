...
class GlyphSyncer(QObject):
    ...
    def start_scanning_loop(self):
        if not self._scan_timer.isActive():
            logger.info("Starting device scanning loop")
    ...
    def stop_scanning_loop(self):
        ...
        logger.info("Stopped device scanning loop and killed subprocesses")
    ...
    def _get_model_async(self, device_id):
        self._run_cmd_async(["-s", device_id, "shell", "getprop", "ro.product.model"])
    ...
    def attempt_connect(self):
        ...
        try:
            ...
            if self._has_unblocked_devices():
                error = Windows.ErrorWindow("Failed to communicate with Phone", str(e))
                error.exec_()
    ...
    def _send_json(self, payload: dict):
        ...
        if not self.client_sock:
            if self._has_unblocked_devices():
                error = Windows.ErrorWindow("Failed to communicate with Phone", "No socket available")
                error.exec_()
    ...
    def set_speed(self, speed: float):
        self._send_json({"action": "set_speed", "value": speed})
