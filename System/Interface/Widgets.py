...
class ValuePopup(QWidget):
    ...
    def setup_animations(self) -> None:
        ...
        self.animation_engine.updated.connect(self.update_position_and_size)

        self.animation_engine.add_properties(
            [
                ("opacity", 1.0,     LoomEngine.MixMode.NOMIX),
                ("rect",    QRect(), LoomEngine.MixMode.NOMIX)
            ]
        )
    ...
    def paintEvent(
        ...
        background_color = QColor(Styles.Colors.secondary_background)
    ...
    def update_position_and_size(self) -> None:
        rect = self.animation_engine.get_property_value("rect")
...
class SegmentedBar(BaseSegmentedBar):
    ...
    def segment_color(self, index: int) -> QColor:
        ...
        if self.hovered_index == index:
            return QColor(Styles.Colors.glass_border).lighter(130)
        
        return QColor(Styles.Colors.glass_border)
...
class GlyphItem(QGraphicsObject):
    ...
    def keyframes(self) -> list | None:
        ...
        if data.get("effect", {}).get("name") == "Fade":
            return data["effect"]["settings"]["keyframes"]
    ...
    def calculate_y_pos(self) -> float:
        ...
        offset     = (Styles.Metrics.Tracks.row_height - Styles.Metrics.Tracks.box_height) / 2
    ...
    def boundingRect(self) -> QRectF:
        ...
        return QRectF(
            ...
            Styles.Metrics.Tracks.box_height + 2 * margin
        )
...
class TrimmingWaveformWidget(QWidget):
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        accent = QColor(Styles.Colors.nothing_accent)
